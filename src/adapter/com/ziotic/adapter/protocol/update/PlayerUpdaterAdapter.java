...
public final class PlayerUpdaterAdapter implements PlayerUpdater {
    ...
    public Frame doApperanceBlock(Player player) {
        ...
        appearanceBlock.writeByte(0 /* 1-mob status */).
        writeByte(app.getPKIcon()).
        writeByte(app.getPrayerIcon()).
        writeByte(0);
        if (!app.isNPC()) {
            for (int i = 0; i < 4; i++) {
    ...
    private void doGraphics2(Player player, FrameBuilder fb) {
        ...
        fb.writeLEShortA(graphic.getId()).
        writeInt1(graphic.getSettings()).
        writeByteS(graphic.getDirection());
    }
