...
public final class ProtocolAdapter implements Protocol {
    ...
    public Protocol sendFriends(Player player, String... friends) {
        Clan clan = player.getOwnClan(); //done
    ...
    public Protocol sendIncommingPM(Player player, String sender, int rights, String message, long id) {
        ...
        fb.writeString(sender); //done?
        fb.writeShort((int) (id >> 32)).
        writeTriByte((int) (id - ((id >> 32) << 32)));
        fb.writeByte(rights);
    ...
    public Protocol sendOutgoingPM(Player player, String recipient, String message) {
        ...
        byte[] textBuffer = new byte[256]; //done?
        textBuffer[0] = (byte) message.length();
        int length = Text.huffmanCompress(message, textBuffer, 1);
    ...
    @Override
    public Protocol sendWorldList(IoSession session) {  //done
    ...
    public Protocol sendResizableScreen(Player player) {
        ...
        sendInterface(player, 751, 746, 16, true); //done
        sendInterface(player, 752, 746, 69, true); //dpme
        sendInterface(player, 754, 746, 70, true); //done
        sendInterface(player, 748, 746, 174, true); //done
        sendInterface(player, 749, 746, 175, true); //done
        sendInterface(player, 750, 746, 176, true); //done run
        sendInterface(player, 747, 746, 177, true); //suummon done?
        sendInterface(player, 745, 746, 12, true); //wtf is this?
    ...
    public Protocol sendInterface(Player player, int id, int window, int location, boolean walkable) {
        ...
        fb.writeLEShort(id).
        writeLEInt(window << 16 | location).
        writeByte(walkable ? 1 : 0); //done
        player.getSession().write(fb.toFrame());
        return this;
    }
    /* (non-Javadoc)
    ...
    @Override
    public Protocol sendCloseInterface(Player player, int window, int location) { //done
    ...
    public Protocol sendAccessMask(Player player, int range1, int range2, int interfaceId1, int childId1, int interfaceId2, int childId2) {
        ...
        fb.writeLEShortA(range2).
        writeLEShortA(range1).
        writeLEInt(interfaceId1 << 16 | childId1).
    ...
    @Override
    public Protocol sendPublicChat(Player player, Player p2, Chat chat) { //done
        ...
        fb.writeShort(p2.getIndex()).
        writeShort(effects).
        writeByte(p2.getRights().
        intValue()).write(textBuffer, 0, length + 1);
        player.getSession().write(fb.toFrame());
        return this;
    }
    ...
    public Protocol sendItems(Player player, int position, boolean posBoolean, PossesedItem[] items) {
        ...
        fb.writeShort(position).
        writeByte(posBoolean ? 1 : 0).
    ...
    @Override
    public Protocol sendLevel(Player player, int skillId) { //done?
        FrameBuilder fb = new FrameBuilder(57, 6);
        if (skillId == Levels.PRAYER)
            fb.writeByteS((byte) Math.ceil(player.getLevels().getCurrentPrayer())).
            writeInt((int) player.getLevels().getExperience(skillId)).
            writeByteA(skillId);
        else
            fb.writeByteS(player.getLevels().getCurrentLevel(skillId)).
            writeInt((int) player.getLevels().getExperience(skillId)).
            writeByteA(skillId);
        player.getSession().write(fb.toFrame());
        return this;
    ...
    public Protocol sendConfig(Player player, int id, int val) {
        FrameBuilder fb;
        if (val < Byte.MIN_VALUE || val > Byte.MAX_VALUE) { //done
            fb = new FrameBuilder(107, 6).
            		writeLEInt(val).
            		writeLEShortA(id);
        } else {
            fb = new FrameBuilder(89, 3).
            		writeByteA(val).
            		writeShort(id);
        }
        player.getSession().write(fb.toFrame());
    ...
    @Override
    public Protocol sendInterfaceVariable(Player player, int id, int val) { //globalConfig?
        FrameBuilder fb;
        if (val < Byte.MIN_VALUE || val > Byte.MAX_VALUE) {
            fb = new FrameBuilder(119, 6).
            		writeInt1(val).
            		writeLEShortA(id);
        } else {
            fb = new FrameBuilder(24, 3).
            		writeShortA(id).
            		writeByteC(val);
        }
        player.getSession().write(fb.toFrame());
